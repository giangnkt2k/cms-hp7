...
<template>
  <el-dialog :model-value="modelValue" destroy-on-close :title="$t('create-user.dialog.title')" @update:model-value="$emit('update:model-value', $event)">
    <el-form label-position="top" @submit.prevent="createUser">
      <el-form-item prop="username" :label="$t('create-user.form.username.label')" :error="$v.username.$errors[0]?.$message.toString()" :show-message="$v.username.$error">
      ...
      <el-form-item>
        <el-button class="w-full" type="primary" :disabled="$v.$invalid" native-type="submit" :loading="isCreatingUser">
